...
impl Item {
    fn new(name: String, price: u32, needs: Vec<(String,u32)>, time: u32) -> Self {
        Item {
...
fn main() {
    println!("Hello, world!");
    let content = file_to_string("dataJSON".to_string());
...
fn write_json_data(file_name: String, item_map: HashMap<String,Item>) {
    ...
    match serde_json::to_string_pretty(&item_arr) {
        Ok(item_json) => {
            match File::create(format!("./dataFiles/calData/{}{}", file_name, ".txt")) {
                Ok(mut file) => {
...
fn cal_added_value(items: Vec<Item>) -> HashMap<String,Item> {
    ...
    item_map.iter_mut().for_each(|(_, value)| value.need_value = Some(value.needs.iter().filter_map(|(need_name,need_amount)| item_map_clone.get(need_name).map(|item| item.price*need_amount)).sum::<u32>()));
...
fn file_to_string(file_name: String) -> String {
    match fs::read_to_string(format!("./dataFiles/rawJSON/{}.txt", &file_name)) {
        Ok(content) => content,
        Err(e) => {
...
mod test_cases {
    ...
    fn test_cal_added_value() {
        ...
        let s3 = Item::new("3".to_string(), 10, vec![("1".to_string(),1),("2".to_string(),1)],0);
